...
df_top10 = df_top10_number.merge(df_top10_minutes,how='inner',on='region')
df_top10.rename(columns={'region':'Countries'}, inplace=True)

# Function to display the plot in interface and show top 10 translation
def display_international():
    fig, axs = plt.subplots(1,2,figsize=(10, 5))
    sns.barplot(data=df_top10,x='Media number', y='Countries',color='royalblue', ax=axs[0])
    axs[0].set_title('Number of media (in Millions)')
    
    sns.barplot(data=df_top10,x='Total duration', y='Countries',color='royalblue',ax=axs[1])
